"use client";

import { useState } from "react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { EnvelopeSimple, LockKey, Eye, EyeSlash, CircleNotch, WarningCircle } from "@phosphor-icons/react";
import { motion } from "framer-motion";
import { createClient } from "@/lib/supabase/client";
import { getEmailByUsername } from "@/lib/auth/checkUsername";

export default function LoginPage() {
  const router = useRouter();
  const supabaseClient = createClient();

  const [usernameOrEmail, setUsernameOrEmail] = useState("");
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [rememberMe, setRememberMe] = useState(true);
  const [isLoading, setIsLoading] = useState(false);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);

  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    setErrorMessage(null);

    let targetEmail = usernameOrEmail.trim();

    if (!targetEmail.includes("@")) {
      try {
        const foundEmail = await getEmailByUsername(targetEmail);
        if (foundEmail) {
          targetEmail = foundEmail;
        }
      } catch (err) {
        console.error("Username lookup error:", err);
      }
    }

    const { data: signInData, error } = await supabaseClient.auth.signInWithPassword({
      email: targetEmail,
      password,
    });

    if (error) {
      setErrorMessage("Username/Email atau kata sandi salah.");
      setIsLoading(false);
      return;
    }

    let targetUrl = "/dashboard";
    const user = signInData?.user;

    if (user) {
      try {
        const { data: profile } = await supabaseClient
          .from("profiles")
          .select("role")
          .eq("id", user.id)
          .maybeSingle();

        if (profile && String(profile.role).toLowerCase().trim() === "admin") {
          targetUrl = "/admin";
        }
      } catch (err) {
        console.error("Fetch profile role error:", err);
      }
    }

    window.location.href = targetUrl;
  };

  const handleGoogleLogin = async () => {
    setIsLoading(true);
    setErrorMessage(null);
    const { error } = await supabaseClient.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${window.location.origin}/auth/callback?next=/dashboard` },
    });
    if (error) {
      setErrorMessage("Gagal masuk dengan Google. Silakan coba lagi.");
      setIsLoading(false);
    }
  };

  return (
    <motion.div
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}
      className="w-full space-y-3"
    >

      {/* ── Tab Switcher ── */}
      <div className="bg-[#ecefe6] p-1 rounded-full flex gap-1">
        <Link
          href="/login"
          className="flex-1 text-center py-2.5 rounded-full font-bold text-xs bg-[#19382B] text-white transition-all"
        >
          Masuk
        </Link>
        <Link
          href="/register"
          className="flex-1 text-center py-2.5 rounded-full font-semibold text-xs text-[#111111]/50 hover:text-[#111111]/80 transition-all"
        >
          Daftar
        </Link>
      </div>

      {/* ── Error ── */}
      {errorMessage && (
        <motion.div
          initial={{ opacity: 0, scale: 0.96 }}
          animate={{ opacity: 1, scale: 1 }}
          className="bg-red-50 border border-red-100 text-red-600 text-[11px] rounded-2xl px-3.5 py-2.5 flex items-start gap-2"
        >
          <WarningCircle size={15} className="shrink-0 mt-0.5" weight="fill" />
          <span className="leading-relaxed">{errorMessage}</span>
        </motion.div>
      )}

      {/* ── Form ── */}
      <form onSubmit={handleLogin} className="space-y-2.5">

        {/* Email / Username */}
        <div className="bg-[#f8f9f5] border border-black/8 focus-within:border-[#19382B] focus-within:ring-2 focus-within:ring-[#19382B]/10 rounded-full px-4 py-2.5 flex items-center gap-3 transition-all">
          <EnvelopeSimple size={17} className="text-[#19382B]/60 shrink-0" weight="duotone" />
          <div className="flex-1 min-w-0">
            <label className="block text-[8px] font-bold uppercase tracking-widest text-[#111111]/35 leading-none mb-0.5">
              Email / Username
            </label>
            <input
              type="text"
              required
              placeholder="nama@email.com"
              value={usernameOrEmail}
              onChange={(e) => setUsernameOrEmail(e.target.value)}
              className="w-full bg-transparent text-sm font-semibold text-[#111111] outline-none placeholder:text-[#111111]/20 placeholder:font-normal"
            />
          </div>
        </div>

        {/* Password */}
        <div className="bg-[#f8f9f5] border border-black/8 focus-within:border-[#19382B] focus-within:ring-2 focus-within:ring-[#19382B]/10 rounded-full px-4 py-2.5 flex items-center gap-3 transition-all">
          <LockKey size={17} className="text-[#19382B]/60 shrink-0" weight="duotone" />
          <div className="flex-1 min-w-0">
            <label className="block text-[8px] font-bold uppercase tracking-widest text-[#111111]/35 leading-none mb-0.5">
              Kata Sandi
            </label>
            <input
              type={showPassword ? "text" : "password"}
              required
              placeholder="••••••••"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              className="w-full bg-transparent text-sm font-semibold text-[#111111] outline-none placeholder:text-[#111111]/25 placeholder:font-normal"
            />
          </div>
          <button
            type="button"
            onClick={() => setShowPassword(!showPassword)}
            className="text-[#111111]/30 hover:text-[#19382B] transition-colors p-0.5 shrink-0"
            aria-label="Tampilkan sandi"
          >
            {showPassword ? <EyeSlash size={15} /> : <Eye size={15} />}
          </button>
        </div>

        {/* Remember me + Lupa sandi */}
        <div className="flex items-center justify-between text-[11px] px-1">
          <label className="flex items-center gap-2 cursor-pointer select-none text-[#111111]/60 font-medium">
            <input
              type="checkbox"
              checked={rememberMe}
              onChange={(e) => setRememberMe(e.target.checked)}
              className="w-3.5 h-3.5 rounded accent-[#19382B]"
            />
            <span>Ingat saya</span>
          </label>
          <Link
            href="/lupa-sandi"
            className="font-semibold text-[#19382B] hover:underline"
          >
            Lupa Kata Sandi?
          </Link>
        </div>

        {/* Submit */}
        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-[#19382B] hover:bg-[#234A39] disabled:opacity-70 text-white py-3 rounded-full font-bold text-sm transition-all active:scale-[0.99] flex items-center justify-center gap-2 mt-1"
        >
          {isLoading ? (
            <>
              <CircleNotch size={16} className="animate-spin text-[#88d937]" />
              <span>Memproses...</span>
            </>
          ) : (
            <span>Masuk</span>
          )}
        </button>
      </form>

      {/* ── Divider ── */}
      <div className="flex items-center gap-3">
        <div className="flex-1 h-px bg-black/8" />
        <span className="text-[10px] font-semibold text-[#111111]/35 uppercase tracking-widest">
          atau masuk dengan
        </span>
        <div className="flex-1 h-px bg-black/8" />
      </div>

      {/* ── Google ── */}
      <button
        type="button"
        onClick={handleGoogleLogin}
        disabled={isLoading}
        className="w-full bg-white border border-black/8 hover:bg-[#f8f9f5] py-3 rounded-full flex items-center justify-center gap-2.5 font-semibold text-sm text-[#111111] transition-all disabled:opacity-60"
      >
        <svg width="16" height="16" viewBox="0 0 48 48">
          <path fill="#FFC107" d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12c0-6.627,5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24c0,11.045,8.955,20,20,20c11.045,0,20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z" />
          <path fill="#FF3D00" d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z" />
          <path fill="#4CAF50" d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36c-5.202,0-9.619-3.317-11.283-7.946l-6.522,5.025C9.505,39.556,16.227,44,24,44z" />
          <path fill="#1976D2" d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571c0.001-0.001,0.002-0.001,0.003-0.002l6.19,5.238C36.971,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z" />
        </svg>
        <span>Masuk dengan Google</span>
      </button>

    </motion.div>
  );
}
